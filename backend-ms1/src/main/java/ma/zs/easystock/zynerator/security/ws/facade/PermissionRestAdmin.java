...
import ma.zs.easystock.zynerator.security.ws.converter.PermissionConverter;
import ma.zs.easystock.zynerator.security.ws.dto.PermissionDto;
...
public class PermissionRestAdmin  extends AbstractController<Permission, PermissionDto, PermissionCriteria, PermissionAdminService, PermissionConverter> {
    ...
    @Operation(summary = "find by modelPermission reference")
    @GetMapping("model-permission/{reference}/action/{action}")
    public Boolean hasCreatePermission(@PathVariable String reference , @PathVariable String action){
        Boolean permission = service.findByModelPermissionReferenceAndActionPermissionReferenceAndSubAttributeIsNull(reference, action);
        return permission;
    }
